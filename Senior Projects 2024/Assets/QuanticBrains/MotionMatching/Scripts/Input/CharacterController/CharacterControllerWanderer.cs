...
{
    [CreateAssetMenu(menuName = "MotionMatching/Character Controller/Wanderer AI Motion")]
    public class CharacterControllerWanderer : CharacterControllerBase
    {
        [SerializeField] private float runDistanceThreshold = 2.5f;
        [SerializeField] private float stopDistanceThreshold = 0.01f;
        [SerializeField] private bool resetTargetOnReach = true;
        [SerializeField] private float resetTargetCd = 5.0f;
        private WanderTarget _wanderTarget;
        ...
        public override void Initialize(MonoBehaviour mb)
        {
            ...
            _wanderTarget = mb.GetComponent<WanderTarget>();
        ...
        {
            var dir = (_wanderTarget.GetTargetPosition() - MotionMatching.transform.position);
        ...
        private void ManageIdle()
        {
            ...
            if (MotionMatching.currentPlayedQuery[0] != "Walk" &&
                _wanderTarget.GetCurrentDistance(MotionMatching.transform.position) < stopDistanceThreshold)
            {
        ...
        {
            var dist = _wanderTarget.GetCurrentDistance(MotionMatching.transform.position);
            ...
            if (dist < stopDistanceThreshold)
            {
                ...
                if (resetTargetOnReach)
                    MotionMatching.StartCoroutine(ResetWanderTarget());
        ...
        private IEnumerator ResetWanderTarget()
        {
            yield return new WaitForSeconds(resetTargetCd);
            _wanderTarget.SelectRandomTarget();
            _isStopped = false;
        }
        ...
        private void CheckForJump(Collider other)
        {
            ...
            var distToTarget = _wanderTarget.GetCurrentDistance(MotionMatching.transform.position);
